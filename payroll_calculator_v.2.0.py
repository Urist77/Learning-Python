...
def taxes():
    # percentages based on an oklahoma resident with one dependent making $40k per year
    # rates found at: https://smartasset.com/taxes/oklahoma-paycheck-calculator#PMgZeBenV7
    social_security = .062
    medicare = .0145
    federal_income_tax = .0729
    state_income_tax = .0176
    tax_rate = social_security + medicare + federal_income_tax + state_income_tax
    return tax_rate
...
def main():
    ...
    # $330.77 allowance for 1 dependent
    taxable_income = (base_pay - 330.77)
    take_home = base_pay - (taxable_income * taxes())
    print(f'Effective tax rate: {taxes():.2f}')
    print(f'Take home pay after taxes {take_home:.2f}')
